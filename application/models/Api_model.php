...
class Api_model extends CI_Model {
    ...
    function get_gamer($id, $chart){
        if(!is_numeric($id))
            return;

        if($chart){
            $this->smx->where('gamer_id', $id);
            $this->smx->where('song_chart_id', $chart);
	    $this->smx->order_by('score', 'DESC');
            $this->smx->select("id, score as max_score, grade, flags");
            $score_data = $this->smx->get('gamer_scores')->result_array()[0];
        }

        $this->db->where('id', $id);
        $this->db->select("id, username, picture_path");
        $gamer = $this->db->get('user')->result_array()[0];

	$gamer['score_data'] = $score_data;
	return $gamer;
    ...
    function user_list($id, $first, $last){
    	...
    	if(is_numeric($id)) {
        	$query[0]['exp_rank'] = $this->getRankPoints($id, 'total');
        	$query[0]['rank_region'] = $this->getRank($id, 'total', $query[0]['country']);

		$query[0]['exp_weekly_rank'] = $this->getWeeklyRankPoints($id, 'total');
		$query[0]['weekly_rank'] = $this->getWeeklyRank($id, 'total');

                $query[0]['exp_weekly_rank_last'] = $this->getWeeklyRankPoints($id, 'total', 'last');
                $query[0]['weekly_rank_last'] = $this->getWeeklyRank($id, 'total', 'last');

			$query[0]['rank_region_basic'] = $this->getRank($id, 'basic', $query[0]['country']);
			$query[0]['rank_region_easy'] = $this->getRank($id, 'easy', $query[0]['country']);
			$query[0]['rank_region_hard'] = $this->getRank($id, 'hard', $query[0]['country']);
			$query[0]['rank_region_wild'] = $this->getRank($id, 'wild', $query[0]['country']);
			$query[0]['rank_region_full'] = $this->getRank($id, 'full', $query[0]['country']);
			$query[0]['rank_region_dual'] = $this->getRank($id, 'dual', $query[0]['country']);
			$query[0]['rank'] = $this->getRank($id, 'total');
			$query[0]['rank_wild'] = $this->getRank($id, 'wild');
			$query[0]['rank_full'] = $this->getRank($id, 'full');
			$query[0]['rank_dual'] = $this->getRank($id, 'dual');
			$query[0]['rank_basic'] = $this->getRank($id, 'basic');
			$query[0]['rank_easy'] = $this->getRank($id, 'easy');
			$query[0]['rank_hard'] = $this->getRank($id, 'hard');
			$query[0]['exp_rank_wild'] = $this->getRankPoints($id, 'wild');
			$query[0]['exp_rank_dual'] = $this->getRankPoints($id, 'dual');
                        $query[0]['exp_rank_full'] = $this->getRankPoints($id, 'full');
			$query[0]['exp_rank_basic'] = $this->getRankPoints($id, 'basic');
			$query[0]['exp_rank_easy'] = $this->getRankPoints($id, 'easy');
			$query[0]['exp_rank_hard'] = $this->getRankPoints($id, 'hard');

			$query[0]['closet_player'] = $this->getClosestRank($id, $query[0]['exp_rank']);
	...
	function weekly_rank_start($week=NULL) {
		if (gmdate("l") == "Tuesday"){
			$start_day = gmdate("Y-m-d");
		} else  {
			date_default_timezone_set('UTC');
			$start_day = gmdate( "Y-m-d", strtotime("last tuesday"));
		}

		if($week=='prev'){
			$start_day = gmdate("Y-m-d", strtotime("$start_day -1 week"));
		}
		return $start_day;
	}

	function getClosestRank($id, $rank){
		...
		foreach ($results as $result){
			if(($result['rank'] - $rank) == 0 ){
				if ($user == 0)
					return $results[1]['user_id'];
				return $user;
			}
				$user = $result['user_id'];
		}
    ...
    function getUserSteps($userid) {
        $this->smx->where('gamer_id', $userid);
	$this->smx->where('created_at >= DATE_SUB(NOW(), INTERVAL 1 DAY)');
	$query = $this->smx->get('gamer_scores');
	$results = $query->result_array();

	$total = Array();
	$total['songs'] = sizeof($results);
	$total['steps'] = 0;
	foreach($results as $song){
	    $total['steps'] += $song['perfect1']+$song['perfect2']+$song['early']+$song['late'];
	}
